...
public class LoginController {
    ...
    public R login(@RequestBody @Validated(LoginGroup.class) UserParam user, @RequestPlatform String platform, @RequestIP String requestIP) {
        return R.ok(userService.login(user.getUsername(), user.getPassword(), AuthType.USER_PASS, platform, requestIP));
    }

    @PostMapping("/token")
    public R loginByToken(@RequestBody UserParam user, @RequestPlatform String platform) {
        Preconditions.checkArgument(StrUtil.isNotBlank(user.getRefreshToken()), "token 不能为空");
        return R.ok(userService.loginByToken(user.getRefreshToken(), AuthType.REFRESH_TOKEN, platform));
    }
    ...
    @PostMapping("/logout")
    public R logout(@AccessToken String accessToken, @RequestPlatform String platform) {
        userService.logout(accessToken, platform);
        return R.ok();
    }
